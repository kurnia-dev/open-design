...
export interface RegisterChatRoutesDeps extends RouteDeps<
  | "db"
  | "design"
  | "http"
  | "chat"
...
export function registerChatRoutes(app: Express, ctx: RegisterChatRoutesDeps) {
  ...
  const rejectProxyPluginContext = (
  ...
  app.post("/api/runs/:id/tool-result", (req, res) => {
    if (typeof submitToolResultToRun !== "function") {
      return sendApiError(
        ...
        501,
        "NOT_IMPLEMENTED",
        "tool-result wiring is not available",
      );
    }
    ...
    if (
      ...
      body.runId.length > 0 &&
      body.runId !== req.params.id
    ) {
      return sendApiError(
        res,
        400,
        "BAD_REQUEST",
        "runId must match the path run id",
      );
    }
    ...
    const result = submitToolResultToRun(
      req.params.id,
      toolUseId,
      content,
      isError,
    ...
    if (!result || !result.ok) {
      ...
      if (reason === "run_terminal" || reason === "stdin_closed") {
        return sendApiError(
          ...
          410,
          "GONE",
          `run is no longer accepting tool results (${reason})`,
        );
      }
      if (reason === "stdin_text_mode") {
        return sendApiError(
          ...
          400,
          "BAD_REQUEST",
          "run does not support interactive tool results",
        );
      }
      ...
      return sendApiError(
        ...
        500,
        "INTERNAL",
        `tool result write failed: ${reason}`,
      );
    }
    res.json({ ok: true });
  });
  ...
  app.post("/api/runs/:id/feedback", async (req, res) => {
    ...
    const reasonCodes = Array.isArray(body.reasonCodes)
      ...
      : [];
    const customReason =
      typeof body.customReason === "string" ? body.customReason : "";
        ...
        try {
          ...
          const safeReasoning =
            ...
            Array.isArray(def.reasoningOptions)
              ? (def.reasoningOptions.find((r: any) => r.id === body.reasoning)
                  ?.id ?? undefined)
              : undefined;
  ...
  app.post("/api/proxy/anthropic/stream", async (req, res) => {
    ...
    return runAnthropicChatStream(res, {
      ...
      payload: buildAnthropicChatPayload(
        model,
        systemPrompt,
        messages,
        maxTokens,
  ...
  app.post("/api/proxy/azure/stream", async (req, res) => {
    ...
    try {
      ...
      let ended = false;
      const guard = createDeltaGuard(sse);
      await streamUpstreamSse(
        response,
        ({ payload: ssePayload, data }: any) => {
          if (ssePayload === "[DONE]") {
            sse.send("end", {});
            ended = true;
            return true;
