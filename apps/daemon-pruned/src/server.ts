import { AGUIEvent } from "@open-design/agui-adapter";
import {
  agentIdToTracking,
...
import { AppVersionInfo, readCurrentAppVersionInfo } from "./app-version.js";
...
import { getDevServerUrl, killPortProcesses } from "./dev-server.js";
...
import { createGitHubRepository, getGitHubToken } from "./github-tokens.js";
import {
  EmptyTranscriptError,
  synthesizeHandoffPrompt,
} from "./handoff-design.js";
import { resolveProjectRelativePath } from "./home-expansion.js";
import {
  registerFinalizeRoutes,
  registerImportRoutes,
  registerProjectExportRoutes,
} from "./import-export-routes.js";
...
import {
  ProjectWatchEvent,
  subscribe as subscribeFileEvents,
} from "./project-watchers.js";
import {
...
export function resolveDaemonCliPath(
  ...
  const packageJsonPath =
    require.resolve("@open-design/daemon-pruned/package.json");
...
export function composeLiveInstructionPrompt({
  ...
  daemonSystemPrompt?: any;
  runtimeToolPrompt?: any;
  clientSystemPrompt?: any;
  finalPromptOverride?: any;
}) {
  const override =
...
export function resolveGrantedCodexImagegenOverride({
  ...
  return renderCodexImagegenOverride(agentId, metadata as any);
}
...
export function normalizeCommentAttachments(input: any) {
  ...
  return input
    .map((raw, index) => {
      ...
      const imageAttachments = normalizePreviewCommentImageAttachments(
        raw.imageAttachments,
      );
      const commentContext =
        raw.commentContext === "query" ? "query" : "context";
      const comment =
        commentContext === "query"
          ? ""
          : cleanString(raw.comment) ||
            intent ||
            imageOnlyCommentFallback(imageAttachments.length);
      const selectionKind =
...
export function renderCommentAttachmentHint(commentAttachments: any[]) {
  ...
  for (const item of commentAttachments) {
    ...
    if (targetKind === "pod") {
      ...
      item.podMembers
        .slice(0, 8)
        .forEach((member: any, memberIndex: number) => {
          lines.push(
          ...
          if (memberStyle)
            lines.push(
              `member.${memberIndex + 1}.computedStyle: ${memberStyle}`,
            );
        });
    }
    const imageAttachments = normalizePreviewCommentImageAttachments(
    ...
    if (imageAttachments.length > 0) {
      ...
      imageAttachments.forEach((attachment: any, attachmentIndex: number) => {
        lines.push(
          `image.${attachmentIndex + 1}: ${attachment.path} | ${attachment.name}`,
        );
      });
    }
  }
...
function normalizePreviewCommentImageAttachments(input: any) {
  if (!Array.isArray(input)) return [];
  const out: any[] = [];
  const seen = new Set();
...
function normalizeVisualMarkKind(value: any) {
  return value === "click" || value === "click+stroke" || value === "stroke"
    ? value
    : "stroke";
}

function visualAnnotationIntent(markKind: any) {
  if (markKind === "click") {
...
function compactString(value: any, max: number) {
...
function normalizeAnnotationStyle(input: any) {
...
function formatAnnotationStyle(style: any) {
...
function finiteAttachmentNumber(value: any) {
...
function formatAttachmentPosition(position: any) {
...
export function resolveSafeProjectAttachments(
  ...
  attachments: any,
  opts: { pathImpl?: typeof path; existsSync?: typeof fs.existsSync } = {},
) {
...
export function formatProjectAttachmentHint(attachments: any[]) {
...
function formatDesignFilesEntryLine(
  entry: DesignFilesHintEntry | null | undefined,
  fallbackKind?: string,
) {
  const entryPath =
  ...
  const size =
    kind === "folder" ? "" : formatProjectEntrySize(Number(entry.size));
...
export function formatDesignFilesWorkspaceHint(
  ...
  if (folderLines.length > 0) {
    ...
    if (omittedFolders > 0)
      lines.push(
        `- ... ${omittedFolders} more folder${omittedFolders === 1 ? "" : "s"} omitted`,
      );
  }
  ...
  if (fileLines.length > 0) {
    ...
    if (omittedFiles > 0)
      lines.push(
        `- ... ${omittedFiles} more file${omittedFiles === 1 ? "" : "s"} omitted`,
      );
  }
...
function resolveDaemonResourceDir(
...
export function isStaticSpaFallbackRequest(req: express.Request) {
  ...
  if (req.path === "/artifacts" || req.path.startsWith("/artifacts/"))
    return false;
...
export function resolveStaticSpaFallbackPath(
  req: express.Request,
  staticDir: string,
) {
  const indexPath = path.join(staticDir, "index.html");
  if (!fs.existsSync(indexPath) || !isStaticSpaFallbackRequest(req))
    return null;
  return indexPath;
}
...
const SANDBOX_RUNTIME = resolveSandboxRuntimeConfig(
...
const USER_DESIGN_TEMPLATES_DIR = path.join(
  RUNTIME_DATA_DIR,
  "design-templates",
);
...
function mcpOAuthCallbackUrl(req: express.Request) {
...
async function refreshAndPersistToken(
  ...
  await setToken(dataDir, serverId, next as any);
  return next;
}

const activeChatAgentEventSinks = new Map<string, (payload: any) => void>();
const activeProjectEventSinks = new Map<string, Set<(payload: any) => void>>();
...
function emitChatAgentEvent(runId: string, payload: any) {
...
function emitLiveArtifactEvent(grant: any, action: any, artifact: any) {
...
function emitProjectEvent(projectId: string | null | undefined, payload: any) {
...
export function createAgentRuntimeEnv(
  ...
  if (nodeBinDir) {
    ...
    const existingPath =
      typeof env[pathKey] === "string" ? (env[pathKey] as string) : "";
...
function normalizeWorkspaceContextItems(items: any) {
...
function mergeRunContextSelections(
  ...contexts: (
    | { pluginIds?: never; mcpServerIds?: never; connectorIds?: never }
    | { pluginIds: any; mcpServerIds: any; connectorIds: any }
  )[]
) {
  ...
  for (const context of contexts) {
    const normalized = normalizeRunContextSelection(context as any);
...
function projectMetadataContextSelection(metadata: {
  contextPlugins: any[];
  contextMcpServers: any[];
  contextConnectors: any[];
}) {
...
function formatContextRefList(ids: any[], refs: any, titleKey = "title") {
  const byId = new Map();
...
function formatWorkspaceContextList(items: any) {
  if (!Array.isArray(items)) return "";
  return items
    .map((item, index) => {
      const details = [
...
function renderWorkspaceContextToolHints(items: any[]) {
...
function renderRunContextPrompt(
  selection: any,
  metadata: {
    contextPlugins: any;
    contextMcpServers: any;
  ...
  const context = mergeRunContextSelections(
    projectMetadataContextSelection(metadata),
    selection,
  );
  const lines: string[] = [];
  if (
    Array.isArray(context.workspaceItems) &&
    context.workspaceItems.length > 0
  ) {
...
export function composeProjectDisplayStatus(
  baseStatus: { value: string },
  awaitingInputProjects: { has: (arg0: any) => any },
  projectId: any,
) {
  if (
 ...
 * @returns {ApiError}
 */
export function createCompatApiError(code: any, message: any, init = {}) {
...
async function readProjectPluginManifest(folder: string) {
...
function scanRunEventsForFinishedProps(
...
export function __forTestScanRunEventsForFinishedProps(
  events: any,
  reqBodyModel: any,
) {
...
function scanRunEventsForRetrySideEffects(events: readonly RunEventLike[]) {
  const sideEffects = {
...
export function __forTestScanRunEventsForRetrySideEffects(events: any) {
  return scanRunEventsForRetrySideEffects(events);
}

function retryFinalResultForRunStatus(
  status: string | undefined,
  retryAttemptCount: any,
) {
...
export function __forTestRetryFinalResultForRunStatus(
  status: any,
  retryAttemptCount: any,
) {
...
function runRetryEventsForAnalytics(events: any) {
  return (Array.isArray(events) ? events : []).filter(
    (rec) =>
      rec?.event === "run_retry_attempted" ||
      rec?.event === "run_retry_finished",
  );
}

export function __forTestRunRetryEventsForAnalytics(events: any) {
...
function reconcileAssistantMessageOnRunEnd(
  ...
  runs: {
    ...
    start?: (run: any, starter: any) => any;
    get?: (id: any) => any;
    list?: ({ projectId, conversationId, status }?: any) => any[];
    stream?: (run: any, req: any, res: any) => void;
    cancel?: (run: any) => void;
    shutdownActive?: ({
      graceMs,
    }?: {
    ...
    emit?: (
      ...
      event: any,
      data: any,
    ) => { id: number; event: any; data: any; timestamp: number };
    finish?: (
      run: any,
      status: any,
      code?: number | null,
    ...
    fail?: (run: any, code: any, message: any, init?: {}) => void;
    drop?: (run: any) => void;
    statusBody?: (run: any) => {
      id: any;
      projectId: any;
  ...
  run: { assistantMessageId: any },
) {
  if (!run.assistantMessageId) return;
  void runs
    .wait(run)
    .then((finalStatus: { status: any }) => {
      db.prepare(
        `UPDATE messages
...
function questionFormBodyIsRenderable(body: string) {
...
function findQuestionFormCloseTag(
  text: string,
  from: number,
  closeTag: string,
) {
...
function emittedRenderableQuestionForm(text: string) {
...
function persistRunEventToAssistantMessage(
  db,
  run: { assistantMessageId: string },
  event: any,
  data: any,
) {
  if (!run.assistantMessageId) return;
...
function runSseEventToPersistedAgentEvent(
  event: string,
  data: { bin: any; chunk: any; error: { message: any }; message: any },
) {
  if (event === "start") {
    return {
      kind: "status",
  ...
  return daemonAgentPayloadToPersistedAgentEvent(data as any);
}
...
function normalizePersistedToolInput(input: { filePath: any }) {
...
function pinAssistantMessageOnRunCreate(
  db,
  run: {
...
export function shouldReportRunCompletedFromMessage(
  saved: { runId: any; runStatus: string },
  body = {} as any,
) {
  return Boolean(
    saved &&
...
function formAnswerTransitionForCurrentPrompt(currentPrompt: string) {
  ...
  if (
    formId.toLowerCase() === "discovery" ||
    formId.toLowerCase() === "task-type"
  ) {
    lines.push(
...
export function createFinalizedMessageTelemetryReporter({
  ...
  return (saved: { runId: any; runStatus: any; endedAt: any }, body = {}) => {
...
function cloudflarePagesDeploymentMetadata(projectName: string) {
...
function cloudflarePagesProjectNameFromUrl(
  rawUrl: string | { toString: () => string },
) {
...
function cloudflarePagesProjectNameForDeploy(
  ...
  const priorName = cloudflarePagesProjectNameFromDeployment(prior as any);
...
async function checkCloudflarePagesDeploymentLinks(existing: {
  ...
  if (customDomain?.url && customDomain.status !== "conflict") {
    ...
    const pagesDomainStatus =
      (pagesDomain as any)?.status || customDomain.pagesDomainStatus;
    ...
    customDomain = {
      ...
      domainStatus: pagesDomain
        ? (pagesDomain as any).status === "active"
          ? "active"
          : failedByApi
            ? "failed"
            : "pending"
      ...
      pagesDomainStatus,
      validationData:
        (pagesDomain as any)?.validation_data ?? customDomain.validationData,
      verificationData:
        (pagesDomain as any)?.verification_data ??
        customDomain.verificationData,
...
function sanitizeArchiveFilename(raw: any) {
...
function sendLiveArtifactRouteError(res, err: unknown) {
...
function normalizeLocalAuthority(value: string) {
...
function isLoopbackPeerAddress(address: string | undefined) {
...
const PROJECT_PREVIEW_ASSET_PATH_RE =
  /^\/projects\/([^/]+)\/preview\/([^/]+)\/.+$/u;
...
function createProjectPreviewScopeRegistry() {
  ...
  return {
    ...
    validate(projectId: string, scope: string | undefined) {
...
function parseProjectPreviewAssetPath(pathname: string) {
  const match = PROJECT_PREVIEW_ASSET_PATH_RE.exec(String(pathname || ""));
  if (!match) return null;
  try {
    return {
      projectId: decodeURIComponent(match[1] as string),
      scope: match[2],
    };
  } catch {
    return null;
...
function localOriginFromHeader(value: string) {
  if (typeof value !== "string") return null;
  const trimmed = value.trim();
...
function requireLocalDaemonRequest(
  req: any,
 ...
 */
function renderOAuthResultPage(opts: { ok: any; serverId: any; message: any }) {
...
const OPEN_DESIGN_GITHUB_REPO_API =
  "https://api.github.com/repos/nexu-io/open-design";
const OPEN_DESIGN_GITHUB_RELEASE_LATEST_API =
...
let openDesignGithubRepoCache: {
  fetchedAt: any;
  stargazersCount?: any;
} | null = null;
let openDesignGithubRepoInflight: Promise<any> | null = null;
let openDesignGithubLatestReleaseCache: {
  fetchedAt: any;
  tagName?: any;
  htmlUrl?: any;
} | null = null;
let openDesignGithubLatestReleaseInflight: Promise<any> | null = null;
let openDesignDiscordPresenceCache: {
  fetchedAt: any;
  onlineCount?: any;
  memberCount?: any;
} | null = null;
let openDesignDiscordPresenceInflight: Promise<any> | null = null;
...
async function readOpenDesignGithubRepoStats() {
  ...
  openDesignGithubRepoInflight = (async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `GitHub repo metadata request failed with HTTP ${response.status}`,
        );
      }
      const payload = await response.json();
      const count =
        payload && typeof (payload as any).stargazers_count === "number"
          ? (payload as any).stargazers_count
          : null;
      if (!Number.isFinite(count) || count == null || count < 0) {
        throw new Error(
          "GitHub repo metadata did not include a numeric stargazers_count",
        );
      }
      openDesignGithubRepoCache = {
...
async function readOpenDesignLatestReleaseInfo() {
  ...
  if (
    ...
    now - openDesignGithubLatestReleaseCache.fetchedAt <
      OPEN_DESIGN_GITHUB_CACHE_TTL_MS
  ) {
  ...
  openDesignGithubLatestReleaseInflight = (async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `GitHub latest release request failed with HTTP ${response.status}`,
        );
      }
      const payload = await response.json();
      const tagName =
        payload && typeof (payload as any).tag_name === "string"
          ? (payload as any).tag_name
          : null;
      const htmlUrl =
        payload && typeof (payload as any).html_url === "string"
          ? (payload as any).html_url
          : null;
      if (!tagName || !htmlUrl) {
        throw new Error(
          "GitHub latest release metadata did not include tag_name/html_url",
        );
      }
      openDesignGithubLatestReleaseCache = {
        tagName,
...
async function readOpenDesignDiscordPresence() {
  ...
  if (
    ...
    now - openDesignDiscordPresenceCache.fetchedAt <
      OPEN_DESIGN_DISCORD_CACHE_TTL_MS
  ) {
  ...
  openDesignDiscordPresenceInflight = (async () => {
    ...
    const timeout = setTimeout(
      () => ctrl.abort(),
      OPEN_DESIGN_DISCORD_TIMEOUT_MS,
    );
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Discord invite metadata request failed with HTTP ${response.status}`,
        );
      }
      const payload = await response.json();
      const profile =
        payload && typeof (payload as any).profile === "object"
          ? (payload as any).profile
          : null;
      const onlineCount =
        typeof (payload as any)?.approximate_presence_count === "number"
          ? (payload as any).approximate_presence_count
          : typeof profile?.online_count === "number"
            ? profile.online_count
            : null;
      const memberCount =
        typeof (payload as any)?.approximate_member_count === "number"
          ? (payload as any).approximate_member_count
      ...
      if (
        ...
        throw new Error(
          "Discord invite metadata did not include numeric member counts",
        );
      }
...
function bearerTokenFromRequest(req: { get: (arg0: string) => any }) {
...
function authorizeToolRequest(req, res, operation: string) {
...
function optionalToolGrantFromRequest(req: any, options = {}) {
...
function requestProjectOverride(
  projectId: string | any[] | undefined,
...
function requestRunOverride(runId: string | any[], tokenRunId: string) {
 ...
 * @param {Omit<ApiError, 'code' | 'message'>} [init]
 */
function createSseErrorPayload(code: string, message: string, init = {}) {
  return { message, error: createCompatApiError(code, message, init) };
}

const MAX_CHAT_IMAGE_BYTES = 1024 * 1024;

function rewriteKnownAgentStreamError(
  agentId: string,
  message: string,
  failureText = "",
) {
  ...
  storage: multer.diskStorage({
    destination: async (req, _file, cb) => {
      try {
        ...
        const meta = projectMetadataLookup?.(req.params.id as string) ?? null;
...
function handleProjectUpload(req, res, next: () => void) {
...
function sendMulterError(res, err: { code: string }) {
...
export function createSseResponse(
  ...
  return {
    ...
    send(
      event: string,
      data: ProjectWatchEvent | AGUIEvent,
...
export function classifyChatRunCloseStatus(params: {
  ...
  if (
    params.code != null &&
    params.code !== 0 &&
    params.artifactProducedThisRun
  ) {
    return "succeeded";
  }
...
`;
  ...
  verdaccioChild = spawn(
    "pnpm",
    ["dlx", "verdaccio", "--config", configPath, "--listen", "0.0.0.0:4873"],
    {
      cwd: configDir,
...
export async function startServer({
  ...
  async function listAllDesignSystems() {
    ...
    try {
      ...
      installed = listed.map((s) => {
        const dirId = s.id.startsWith("user:")
          ? s.id.slice("user:".length)
          : s.id;
  ...
  async function readAvailableDesignSystem(id: string) {
    if (typeof id === "string" && id.startsWith("user:")) {
      return readDesignSystem(USER_DESIGN_SYSTEMS_DIR, id, {
        idPrefix: "user:",
      });
    }
    return (
  ...
  async function readAvailableDesignSystemPackageInfo(id: string) {
    if (typeof id === "string" && id.startsWith("user:")) {
      return readDesignSystemPackageInfo(USER_DESIGN_SYSTEMS_DIR, id, {
        idPrefix: "user:",
      });
    }
    return (
  ...
  function isProjectUsableDesignSystem(summary: { status: string }) {
    return summary?.status !== "draft";
  }

  async function validateProjectDesignSystemId(id: string | null | undefined) {
  ...
  function projectBackedDesignSystemProjectId(
    id: any,
    summary: { projectId: any },
  ) {
  ...
  async function ensureUserDesignSystemWorkspaceProject(db, id: string) {
    const systems = await listAllDesignSystems();
    ...
    for (const file of files) {
      ...
      const detail = await readUserDesignSystemFile(
        USER_DESIGN_SYSTEMS_DIR,
        id,
        file.path,
      );
      ...
      if (existing) {
        try {
          const existingFile = await readProjectFile(
            PROJECTS_DIR,
            projectId,
            detail.path,
            project.metadata,
          ...
          if (
            !isReplaceableDesignSystemWorkspaceFile(detail.path, existingFile)
          )
            continue;
        } catch (err) {
    ...
    if (githubUrl && accessToken) {
      try {
        ...
        if (!hasGit) {
          ...
          const authPushUrl = githubUrl.replace(
            "https://github.com/",
        ...
        console.warn(
          "[server] Failed to push design system workspace project to GitHub:",
          gitErr,
        );
      }
    }
  ...
  function isReplaceableDesignSystemWorkspaceFile(
    filePath: string,
    file: {
      buffer: any;
      name?: any;
      ...
      artifactManifest?: { [x: string]: unknown } | null;
    },
  ) {
    ...
    if (
      /^ui_kits\/app\/components\/.+\.(jsx|tsx|js|ts|css|html)$/u.test(filePath)
    ) {
      return buffer.length < 700 && /od-ui-kit-[a-z-]+/u.test(text);
    }
    if (
      !/^(DESIGN\.md|README\.md|SKILL\.md|ui_kits\/app\/README\.md)$/u.test(
        filePath,
      )
    ) {
      return false;
  ...
  async function removeLegacyDesignSystemWorkspaceArtifacts(project: {
    ...
    for (const artifact of LEGACY_DESIGN_SYSTEM_ARTIFACTS) {
      ...
      await fs.promises.rm(path.join(dir, ...artifact.legacyPath.split("/")), {
        recursive: (artifact as any).removeDirectory === true,
        force: true,
      });
    }
  }
  ...
  const _NULL_ORIGIN_SAFE_GET_RE = /^\/projects\/[^/]+\/(?:raw|preview)\//;
  ...
  // Plugins and marketplaces feature is pruned. Skipping bundled plugin registration, marketplace seeding, and snapshot GC.
      ...
      installedPlugins: (() => {
        try {
          return (
            (
              db
                .prepare("SELECT COUNT(*) AS n FROM installed_plugins")
                .get() as any
            )?.n ?? 0
          );
        } catch {
  ...
  app.post(
    "/api/agents/:agentId/oauth-launch",
    ...
    async (req, res) => {
      ...
      try {
        const { launchAgentInSystemTerminal } =
          await import("./runtimes/terminal-launch.js");
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const { verifySqliteIntegrity } =
          await import("./storage/db-inspect.js");
        const quick = String(req.query.quick ?? "").toLowerCase();
        const report = verifySqliteIntegrity({
          db,
  ...
  app.get("/api/critique/conformance", async (req, res) => {
    try {
      ...
      const cleanParseThreshold = parseRate(
        req.query.cleanParseThreshold,
        0.95,
      );
      const history = await readConformanceHistory(
        RUNTIME_DATA_DIR,
        windowDays,
      );
  ...
  registerMemoryRoutes(app, {
    http: {
      ...
      requireLocalDaemonRequest,
      isLocalSameOrigin,
      resolvedPortRef: resolvedPort,
      sendApiError,
      sendLiveArtifactRouteError,
      sendMulterError,
    } as any,
    paths: { RUNTIME_DATA_DIR, PROJECT_ROOT, PROJECTS_DIR } as any,
  ...
  const design = {
    runs: createChatRunService({
      ...
      runsLogDir: path.join(RUNTIME_DATA_DIR, "runs") as any,
    }),
  ...
  process.on("unhandledRejection", (reason) => {
    ...
    triggerFatalShutdown("daemon_unhandled_rejection", {
      ...
      error_stack:
        typeof asError?.stack === "string"
          ? asError.stack.slice(0, 8192)
          : undefined,
    });
  });
  ...
  void (async () => {
    try {
      ...
      await observePendingInstallerApplyAttempts({
        ...
        logger: console,
        namespace:
          process.env[SIDECAR_ENV.NAMESPACE] ?? SIDECAR_DEFAULTS.namespace,
      });
    } catch {
  ...
  const reportFinalizedMessage = createFinalizedMessageTelemetryReporter({
    ...
    db,
    dataDir: RUNTIME_DATA_DIR,
    reportedRuns: reportedRuns as Set<string>,
  ...
  const validateExternalApiBaseUrl = (baseUrl: string) =>
    validateBaseUrlResolved(baseUrl);
  ...
  app.get("/api/projects/:id/events", (req, res) => {
    ...
    let sub: { ready?: any; unsubscribe: any } | null = null;
    try {
      const sse = createSseResponse(res);
      const projectEventSink = (payload: { type: any }) => {
        sse.send(payload.type, payload as any);
      };
      ...
      sub.ready
        .then(() => sse.send("ready", { projectId: req.params.id } as any))
        .catch(() => {});
      const cleanup = () => {
        if (sub) {
  ...
  app.post("/api/projects/:id/conversations", (req, res) => {
    ...
    const { title, seedFromConversationId, forkAfterMessageId } =
      req.body || {};
    const now = Date.now();
    ...
    if (sourceConversation && sourceConversation.projectId === req.params.id) {
      ...
      return res
        .status(404)
        .json({ error: "fork source conversation not found" });
    }
  ...
  app.put("/api/projects/:id/conversations/:cid/messages/:mid", (req, res) => {
    ...
    reportFinalizedMessage(saved as any, m);
  ...
  app.post("/api/projects/:id/conversations/:cid/comments", (req, res) => {
    ...
    try {
      ...
      res.status(400).json({ error: String((err as any)?.message || err) });
    }
  });
  ...
  app.post("/api/design-systems", async (req, res) => {
    try {
      ...
      if (gitHubRepo) {
        if (!accessToken) {
          return res.status(401).json({
            error: "Not connected to GitHub, cannot create repository",
          });
        }
        try {
          ...
          return res.status(400).json({
            error: `Failed to create GitHub repository: ${repoErr.message || repoErr}`,
          });
        }
      }
  ...
  app.get("/api/design-systems/generation-jobs/:jobId", async (req, res) => {
    try {
      ...
      if (!job) {
        return res
          .status(404)
          .json({ error: "design system generation job not found" });
      }
      res.json({ job });
    } catch (err) {
      res.status(500).json({ error: String(err) });
  ...
  app.post(
    "/api/design-systems/:id/token-contract/rebuild-jobs",
    async (req, res) => {
      try {
        const preparation = await prepareDesignTokenContractRebuild(
          USER_DESIGN_SYSTEMS_DIR,
  ...
  app.get("/api/design-systems/:id/revisions", async (req, res) => {
    try {
      ...
      if (!revisions) {
        return res
          .status(404)
          .json({ error: "editable design system not found" });
      }
      res.json({ revisions });
    } catch (err) {
      res.status(500).json({ error: String(err) });
  ...
  app.patch(
    "/api/design-systems/:id/revisions/:revisionId",
    async (req, res) => {
      try {
        const status =
        ...
        if (!revision) {
          return res
            .status(404)
            .json({ error: "design system revision not found" });
        }
        res.json({ revision });
      } catch (err) {
        res.status(400).json({ error: String(err) });
  ...
  app.get("/api/design-systems/:id", async (req, res) => {
    try {
      ...
      const body =
        projectBody ?? (await readAvailableDesignSystem(req.params.id));
      if (body === null || !summary)
        return res.status(404).json({ error: "design system not found" });
      const packageInfo = await readAvailableDesignSystemPackageInfo(
        req.params.id,
      );
      const detail = {
        ...summary,
  ...
  app.get("/api/design-systems/:id/publish", async (req, res) => {
    ...
    try {
      ...
      if (!updated) {
        writeEvent("publish-error", {
          message: "editable design system not found",
        });
      } else {
  ...
  app.delete("/api/design-systems/:id", async (req, res) => {
    try {
      const dirId = req.params.id.startsWith("user:")
        ? req.params.id.slice("user:".length)
        : req.params.id;
      ...
      if (!force) {
        ...
        if (!dsClean || !projectClean) {
          return res.status(409).json({
            error: "GIT_DIRTY",
            message:
              "Working tree is not clean. Uncommitted changes will be lost if deleted.",
          });
        }
      }

      const ok = await deleteUserDesignSystem(
        USER_DESIGN_SYSTEMS_DIR,
        req.params.id,
      );
      if (!ok) {
  ...
  app.post("/api/runs/:runId/genui/:surfaceId/respond", async (req, res) => {
    try {
      ...
      const row = stmt.get(req.params.runId, req.params.surfaceId) as
        | { id?: string }
        | undefined;
      if (!row?.id) {
        return res
          .status(404)
          .json({ error: "no pending surface for runId/surfaceId" });
      }
  ...
  app.post("/api/projects/:projectId/genui/prefill", (req, res) => {
    try {
      const body = req.body && typeof req.body === "object" ? req.body : {};
      const snapshotId =
        typeof body.snapshotId === "string" ? body.snapshotId : "";
      const surfaceId =
        typeof body.surfaceId === "string" ? body.surfaceId : "";
      const persist =
        body.persist === "run" ||
        body.persist === "conversation" ||
        body.persist === "project"
          ? body.persist
          : "project";
      const kind =
        body.kind === "form" ||
        body.kind === "choice" ||
        body.kind === "oauth-prompt"
          ? body.kind
          : "confirmation";
      if (!snapshotId || !surfaceId) {
        return res
          .status(400)
          .json({ error: "snapshotId and surfaceId are required" });
      }
  ...
  app.get("/api/runs/:runId/genui/:surfaceId", (req, res) => {
    try {
      const row = db
        ...
        .get(req.params.runId, req.params.surfaceId) as
        | { id?: string }
        | undefined;
  ...
  app.post("/api/upload", upload.array("images", 8), (req, res) => {
    const files = ((req.files || []) as any).map(
      (f: { originalname: any; path: any; size: any }) => ({
        name: f.originalname,
        path: f.path,
        size: f.size,
      }),
  ...
  app.post("/api/projects/:id/deploy", async (req, res) => {
    try {
      ...
      const body = upsertDeployment(db, {
        ...
        cloudflarePages: (result as any).cloudflarePages,
        providerMetadata:
          providerId === CLOUDFLARE_PAGES_PROVIDER_ID
  ...
  app.post("/api/projects/:id/finalize/anthropic", async (req, res) => {
    ...
    try {
      ...
      const safeMsg = redactSecrets(String((err as any)?.message || err), [
        apiKey,
      ]);
  ...
  app.get(/^\/api\/projects\/([^/]+)\/raw\/(.+)$/u, async (req, res) => {
    try {
      ...
      const status = err && (err as any).code === "ENOENT" ? 404 : 400;
      sendApiError(
        res,
        status,
        status === 404 ? "FILE_NOT_FOUND" : "BAD_REQUEST",
  ...
  app.get("/api/projects/:id/files/:name/preview", async (req, res) => {
    try {
      ...
      sendApiError(
        ...
        status,
        status === 404 ? "FILE_NOT_FOUND" : "BAD_REQUEST",
        (err as any)?.message || "preview unavailable",
      );
    }
  });
  ...
  app.post("/api/orbit/run", async (req, res) => {
    ...
    try {
      const locale =
        typeof req.body?.locale === "string" ? req.body.locale : null;
  ...
  app.post("/api/system/open-external", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ ok: false, error: "url must be a valid URL" });
      }
      if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        return res
          .status(400)
          .json({ ok: false, error: "url must be http or https" });
      }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const relDir =
          typeof (req as any)._uploadRelDir === "string"
            ? (req as any)._uploadRelDir
            : "";
        const out: any[] = [];
  ...
  const composeDaemonSystemPrompt = async ({
    ...
    let skillName;
    let skillMode;
    const skillModes = new Set<
      NonNullable<Parameters<typeof composeSystemPrompt>[0]["skillMode"]>
    >();
    ...
    const mergeSkillCritiquePolicy = (
      ...
      if (next === "required")
        return current === "opt-out" ? current : "required";
      if (next === "opt-in") {
        return current === "required" || current === "opt-out"
          ? current
          : "opt-in";
      }
      return current;
    ...
    if (adHocSkillIds.length > 0) {
      ...
      const blocks: string[] = [];
      const baseBody =
        skillBody && skillBody.trim().length > 0 ? skillBody : "";
      for (const id of adHocSkillIds) {
        ...
        if (!critiqueSkillId || extra.critiquePolicy !== null)
          critiqueSkillId = canonicalId;
        skillCritiquePolicy = mergeSkillCritiquePolicy(
          skillCritiquePolicy,
        ...
        if (Array.isArray(extra.craftRequires)) {
          for (const craft of extra.craftRequires) {
            if (!skillCraftRequires.includes(craft))
              skillCraftRequires.push(craft);
          }
        }
        blocks.push(
    ...
    let designSystemCraftExemptions: Iterable<any> | null | undefined = [];
    if (effectiveDesignSystemId) {
      ...
      if (
        summary &&
        (isProjectUsableDesignSystem(summary) || editingOwnDraftDesignSystem)
      ) {
        ...
        designSystemCraftApplies = Array.isArray(assets.craftApplies)
          ? assets.craftApplies
          : [];
        designSystemCraftExemptions = Array.isArray(assets.craftExemptions)
          ? assets.craftExemptions
          : [];
      }
    }
    ...
    const audioVoiceOptions = undefined;
    const audioVoiceOptionsError = undefined;
    const template =
    ...
    const critiqueBrand =
      ...
      typeof designSystemBody === "string"
        ...
        : undefined;
    const critiqueSkill =
      critiqueEnabledForRun && typeof critiqueSkillId === "string"
        ? { id: critiqueSkillId }
        : undefined;
    ...
    const prompt = composeSystemPrompt({
      ...
      critique: critiqueShouldRun
        ? { ...critiqueCfg, enabled: true }
        : undefined,
  ...
  const startChatRun = async (
    ...
    run: {
      ...
      promptTelemetry?: any;
      stdinOpen?: any;
      pendingHostAnswers?: any;
      retryFinalResult?: any;
      retrySuppressedReason?: any;
      turnCompletedCleanly?: any;
    },
  ) => {
    ...
    let existingProjectFolders: any[] = [];
    if (typeof projectId === "string" && projectId) {
      try {
        ...
        existingProjectFolders = await listProjectFolders(
          PROJECTS_DIR,
          projectId,
          { metadata: chatMeta },
        );
    ...
    const projectRecord =
      typeof projectId === "string" && projectId
        ? getProject(db, projectId)
        : null;
    const runContextPrompt = renderRunContextPrompt(
      context,
    ...
    const revokeToolToken = (reason: string | undefined) => {
      if (toolTokenRevoked || !toolTokenGrant) return;
      toolTokenRevoked = true;
      toolTokenRegistry.revokeToken(toolTokenGrant.token, reason as any);
    };
    ...
    let externalMcpConfig: any = { servers: [] };
    if (!SANDBOX_RUNTIME.enabled) {
      try {
    ...
    const runScopedMcpServers = Array.isArray(run?.toolBundle?.mcpServers)
      ...
      : [];
    const { enabledServers: enabledExternalMcp, persistedTokenServerIds } =
      resolveExternalMcpServersForRun({
    ...
    const stableInstructionFingerprint = [
      ...
      .map((part) => (typeof part === "string" ? part.trim() : ""))
      .join("\n\n---\n\n");
    const currentStableHash = hashStableInstructions(
    ...
    const formIdForOverride = formAnswerMatch
      ? (
          (formAnswerMatch[1] || "form").trim().replace(/[^\w.-]/g, "") ||
          "form"
        ).toLowerCase()
      : null;
    const formOverride =
    ...
    const composed = [
      ...
      promptImagePaths.length
        ? `\n\n${promptImagePaths.map((p: any) => `@${p}`).join(" ")}`
        : "",
    ].join("");
    console.log(
    ...
    const send = (event: any, data: any) => {
      ...
      persistRunEventToAssistantMessage(db, run as any, event, data);
    ...
    const restartSameRunAfterRetry = () => {
      ...
      run.analyticsTelemetry = {
        startRequestedAt:
          run.analyticsTelemetry?.startRequestedAt ?? run.createdAt,
      };
    ...
    let mmdRouteLaunchEnv: any | null = null;
    ...
    if (antigravityConcreteModel) {
      const { acquireAntigravityModelLock } =
        await import("./runtimes/defs/antigravity.js");
    ...
    let inactivityTimer: string | number | NodeJS.Timeout | null | undefined =
      null;
    ...
    const failForInactivity = () => {
      ...
      let stallPayload: any = null;
      if (agentId === "opencode") {
        const logFailure = readOpenCodeServiceFailure(
          spawnedAgentEnv as Record<string, string | undefined>,
          {
            since: run.createdAt,
          },
        );
    ...
    if (toolTokenGrant?.runId) {
      ...
      activeChatRunHandles.set(toolTokenGrant.runId, {
    ...
    let spawnedAgentEnv: NodeJS.ProcessEnv | null = null;
    let agentStdoutTail = "";
    let agentStderrTail = "";
    try {
      ...
      if (
        ...
        const { waitForAgyToReadModel } =
          await import("./runtimes/defs/antigravity.js");
        void waitForAgyToReadModel(agentLogFilePath, antigravityConcreteModel, {
    ...
    child.stdout?.on("data", (chunk: any) => {
      ...
      if (memoryAssistantBuffer.length > MEMORY_BUFFER_CAP) {
        memoryAssistantBuffer = memoryAssistantBuffer.slice(
          0,
          MEMORY_BUFFER_CAP,
        );
      }
        ...
        const critiqueBus = {
          emit: (e: { event: any; data: any }) => {
            ...
            if (critiqueProjectIdForBus) {
              emitProjectEvent(critiqueProjectIdForBus, {
                ...e.data,
                type: e.event,
              });
            }
    ...
    const noteFirstTokenFromAgentEvent = (ev: any) => {
    ...
    function guardTextDelta(delta: any) {
    ...
    if (def.streamFormat === "claude-stream-json") {
      ...
      child.stdout?.on("data", (chunk: unknown) => qoder.feed(chunk));
      ...
      acpSession = attachPiRpcSession({
        ...
        send: (channel, payload: any) => {
          if (channel === "agent") {
            ...
            if (
              piErrorCode === "PI_PARENT_SESSION_FAILED" &&
              run.conversationId
            ) {
      ...
      acpSession = attachAcpSession({
        ...
        ...(acpStageTimeoutMs !== undefined
          ? { stageTimeoutMs: acpStageTimeoutMs }
          : {}),
      });
    ...
    child.on("close", async (code: number, signal: any) => {
      try {
        clearInactivityWatchdog();
          ...
          if (authFailure?.status === "missing") {
            send(
              ...
              createSseErrorPayload(
                "AGENT_AUTH_REQUIRED",
                authFailure.message ??
                  `${def.name} authentication required. Please re-authenticate and retry.`,
                { retryable: true },
              ),
            );
        ...
        if (
          ...
          const msg = authFailure
            ? (authFailure.message ??
              `${def.name} authentication expired. Please re-authenticate and retry.`)
            : isAntigravityQuota
              ? antigravityQuotaGuidance()
          ...
          send(
            "error",
            createSseErrorPayload(errorCode, msg, { retryable: true }),
          );
          return finishWithRetryDecision("failed", 0, signal);
        }
        ...
        const acpCleanCompletion =
          typeof (acpSession as any)?.completedSuccessfully === "function" &&
          (acpSession as any).completedSuccessfully();
        const runArtifactSideEffects = scanRunEventsForRetrySideEffects(
          run.events,
        );
          ...
          if (diagnostic) {
            ...
            if (openCodeFailure) {
              ...
              if (rewritten !== "Agent stream error") {
                send(
                  "error",
                  createSseErrorPayload("AGENT_EXECUTION_FAILED", rewritten, {
                    retryable: true,
                  }),
                );
              }
            ...
            try {
              ...
              for (const f of files) {
                ...
                try {
                  ...
                  if (!isRunTouchedProjectFile(st.mtimeMs, runStartTimeMs))
                    continue;
        ...
        if (
          acpSession &&
          typeof (acpSession as any).getLastSessionPath === "function"
        ) {
          const sessionPath = (acpSession as any).getLastSessionPath();
    ...
    if (writePromptToChildStdin && child.stdin) {
      ...
      if (promptInputFormat === "stream-json") {
        ...
        try {
          ...
          if (err && (err as any).code !== "EPIPE") throw err;
        }
        run.stdinOpen = true;
      } else {
  ...
  orbitService.setRunHandler(
    async ({ trigger, startedAt, prompt, systemPrompt, template }) => {
      ...
      const completion = (async () => {
        const finalStatus: any = await design.runs.wait(run);
        db.prepare(
  ...
  function runToolBundleDeliveryTargetForProject(
    ...
    try {
      ...
      return isManagedProjectCwd(cwd, PROJECTS_DIR)
        ? "managed-project"
        : "external-project";
    } catch {
      return "none";
  ...
  app.post("/api/runs", async (req, res) => {
    ...
    if (typeof meta.agentId !== "string" || !meta.agentId) {
      try {
        const appCfg = await readAppConfig(RUNTIME_DATA_DIR);
        const cfgAgent =
          typeof appCfg.agentId === "string" && appCfg.agentId
            ? appCfg.agentId
            : null;
    ...
    const run: any = design.runs.create(meta);
    try {
    ...
    if (analyticsContext) {
      ...
      const detectedAgentsForAnalytics = await detectAgents(
        ((appCfgForAnalytics as { agentCliEnv?: Record<string, unknown> })
          .agentCliEnv ?? {}) as any,
      ...
      const userQueryTokens =
        promptText.length > 0 ? Math.ceil(promptText.length / 4) : 0;
      ...
      const analyticsHints =
        (reqBody as { analyticsHints?: Record<string, unknown> | null })
          .analyticsHints &&
        typeof (reqBody as { analyticsHints?: unknown }).analyticsHints ===
          "object"
          ? ((reqBody as { analyticsHints?: Record<string, unknown> })
              .analyticsHints ?? {})
          : {};
      const hintEntryFrom =
        typeof analyticsHints.entryFrom === "string"
          ? analyticsHints.entryFrom
          : undefined;
      const hintProjectKind =
        typeof analyticsHints.projectKind === "string"
          ? analyticsHints.projectKind
          : null;
      const requestProjectId =
        typeof reqBody.projectId === "string" ? reqBody.projectId : null;
      const runProject = requestProjectId
        ? getProject(db, requestProjectId)
        : null;
      const runProjectKind = resolveRunProjectKindForAnalytics({
        hintProjectKind,
      ...
      const baseProps: Record<string, unknown> = {
        ...
        conversation_id:
          typeof reqBody.conversationId === "string"
        ...
        ...(isDesignSystemRun
          ? {
              ds_source_origin:
                typeof dsRunContext.origin === "string"
                  ? dsRunContext.origin
                  : undefined,
              source_count:
                typeof dsRunContext.sourceCount === "number"
                  ? dsRunContext.sourceCount
                  : undefined,
              has_brand_description:
                typeof dsRunContext.hasBrandDescription === "boolean"
              ...
              brand_description_length_bucket:
                typeof dsRunContext.brandDescriptionLengthBucket === "string"
                  ...
                  : undefined,
              github_repo_count:
                typeof dsRunContext.githubRepoCount === "number"
                  ? dsRunContext.githubRepoCount
                  : undefined,
              local_folder_count:
                typeof dsRunContext.localFolderCount === "number"
                  ? dsRunContext.localFolderCount
                  : undefined,
              fig_file_count:
                typeof dsRunContext.figFileCount === "number"
                  ? dsRunContext.figFileCount
                  : undefined,
              asset_file_count:
                typeof dsRunContext.assetFileCount === "number"
      ...
      design.runs
        ...
        .then(async (status: any) => {
          ...
          const langfuseDeliveryForAnalytics = deriveLangfuseDeliveryState(
            (appCfgAtFinish as { telemetry?: Record<string, unknown> })
              .telemetry ?? {},
            readTelemetrySinkConfig(),
          );
          ...
          const finishedModelId = hasExplicitRequestedModelForAnalytics(
            reqBody.model,
          )
            ? modelIdForTracking(reqBody.model)
            : modelIdForTracking(usageAnalytics.agent_reported_model);
          for (const [index, retryEvent] of runRetryEventsForAnalytics(
            run.events,
          ).entries()) {
            design.analytics.capture({
            ...
            properties: {
              ...
              ...(isDesignSystemRun
                ? {
                    ...
                    design_system_created: didRunCreateDesignSystemFile(
                      run.events,
                    ),
                    preview_module_count: countDesignSystemPreviewModules(
                      run.events,
                    ),
              ...
              ...(usageAnalytics.input_tokens_effective !== undefined
                ? {
                    input_tokens_effective:
                      usageAnalytics.input_tokens_effective,
                  }
                : {}),
              ...
              ...(usageAnalytics.cache_read_input_tokens !== undefined
                ? {
                    cache_read_input_tokens:
                      usageAnalytics.cache_read_input_tokens,
                  }
                : {}),
              ...
              ...(usageAnalytics.estimated_context_tokens !== undefined
                ? {
                    estimated_context_tokens:
                      usageAnalytics.estimated_context_tokens,
                  }
                : {}),
  ...
  app.get("/api/runs/:id/agui", async (req, res) => {
    ...
    const lastEventId = Number(
      req.get("Last-Event-ID") || req.query.after || 0,
    );
    const emitMapped = (record: any) => {
    ...
    const adapterClient = {
      send: (event: any, data: any, id: any) => {
        const mapped = encodeOdEventForAgui(
          { kind: event, ...(data ?? {}) },
  ...
  app.post("/api/chat", (req, res) => {
    ...
    const toolBundle = parseRunToolBundleForRequest(
      requestBody.toolBundle,
    ) as any;
    if (!toolBundle.ok) {
      return sendApiError(res, 400, "BAD_REQUEST", toolBundle.message);
    }
    let chatProject: any = null;
    ...
    const toolBundleSupport = validateRunToolBundleForAgent(
      ...
      typeof requestBody.agentId === "string"
        ? getAgentDef(requestBody.agentId)
        : null,
      {
        deliveryTarget: runToolBundleDeliveryTargetForProject(
...
function sanitizeSlug(text: any) {
  return String(text)
    .toLowerCase()
    .replace(/[^\w\s-]/g, "")
    .replace(/[\s_]+/g, "-")
...
function assembleExample(templateHtml: any, slidesHtml: any, title: any) {
  return templateHtml
