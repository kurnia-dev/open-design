...
type CloudflarePagesDeploySelection = {
  zoneId: string;
  zoneName: string;
  domainPrefix: string;
  hostname: string;
};
type CloudflareDnsRecord = JsonObject & {
  id?: string;
  type?: string;
  name?: string;
  content?: string;
...
export class DeployError extends Error {
  ...
  constructor(
    message: string,
    status = 400,
    details: DeployErrorDetails = undefined,
    code?: string,
  ) {
    super(message);
  ...
  try {
    ...
    return {
      ...
      cloudflarePages: normalizeCloudflarePagesConfigHints(
        parsed.cloudflarePages,
      ),
    };
  } catch (err) {
    if (isErrnoException(err) && err.code === "ENOENT")
      return { token: "", accountId: "", projectName: "", cloudflarePages: {} };
    throw err;
  }
}
...
export async function writeCloudflarePagesConfig(input: Partial<DeployConfig>) {
  ...
  const tokenInput = typeof input?.token === "string" ? input.token.trim() : "";
  const cloudflarePages = normalizeCloudflarePagesConfigHints(
    input?.cloudflarePages,
    current.cloudflarePages,
  );
  ...
  if (Object.keys(cloudflarePages).length > 0)
    next.cloudflarePages = cloudflarePages;
  if (!next.token)
...
export function publicCloudflarePagesConfig(config: Partial<DeployConfig>) {
  ...
  if (Object.keys(cloudflarePages).length > 0)
    body.cloudflarePages = cloudflarePages;
  return body;
}
...
export async function readDeployConfig(
  ...
  if (providerId === CLOUDFLARE_PAGES_PROVIDER_ID)
    return readCloudflarePagesConfig();
...
export async function writeDeployConfig(
  ...
  if (providerId === CLOUDFLARE_PAGES_PROVIDER_ID)
    return writeCloudflarePagesConfig(input);
...
export function publicDeployConfigForProvider(
  ...
  if (providerId === CLOUDFLARE_PAGES_PROVIDER_ID)
    return publicCloudflarePagesConfig(config);
...
function normalizeCloudflarePagesConfigHints(
  input: unknown,
  fallback: CloudflarePagesConfigHints = {},
...
export async function buildDeployFilePlan(
  projectsRoot: string,
  projectId: string,
  entryName: string,
  options: DeployOptions = {},
): Promise<DeployFilePlan> {
  ...
  const pending: { ref: string; base: string }[] = extractHtmlReferences(
    html,
  ).map((ref) => ({
    ref,
    base: entryBase,
  ...
  const supportingFiles =
    "supportingFiles" in (entry.artifactManifest ?? {})
      ? ((entry.artifactManifest as { supportingFiles?: string[] })
          .supportingFiles ?? [])
      : [];
  ...
  while (pending.length > 0) {
    ...
    let projectFile;
    try {
      projectFile = await readProjectFile(
        projectsRoot,
        projectId,
        safePath,
        options.metadata,
    ...
    if (/\.css$/i.test(safePath)) {
      ...
      for (const ref of extractCssReferences(
        projectFile.buffer.toString("utf8"),
      )) {
...
export async function buildDeployFileSet(
  projectsRoot: string,
  projectId: string,
  entryName: string,
  options: DeployOptions = {},
) {
  const plan = await buildDeployFilePlan(
    projectsRoot,
    projectId,
    entryName,
    options,
...
async function addVisibleProjectFilesToDeployPlan(
  ...
  const projectFiles: any[] = await listFiles(
    input.projectsRoot,
    input.projectId,
    {
...
export async function deployToCloudflarePages(input: {
  ...
  return {
    ...
    providerMetadata: cloudflarePagesProviderMetadata(
      config.projectName,
      cloudflarePagesInfo,
      { projectId },
    ),
...
function normalizeCloudflarePagesDeploySelection(
  input: unknown,
): CloudflarePagesDeploySelection | null {
  if (!input || typeof input !== "object") return null;
  const source = input as JsonObject;
  const rawZoneId =
    typeof source.zoneId === "string" ? source.zoneId.trim() : "";
  const rawZoneName =
    typeof source.zoneName === "string" ? source.zoneName.trim() : "";
  const rawPrefix =
    typeof source.domainPrefix === "string" ? source.domainPrefix.trim() : "";
...
async function validateCloudflarePagesDeploySelection(
  config: DeployConfig,
  selection: CloudflarePagesDeploySelection | null,
): Promise<CloudflarePagesDeploySelection | null> {
...
async function setupCloudflarePagesCustomDomain({
  ...
  config: DeployConfig;
  projectId: string;
  selection: CloudflarePagesDeploySelection;
  pagesDevUrl: string;
  priorMetadata?: JsonObject | undefined;
}) {
  if (!config.projectName)
  ...
  const pagesTarget = normalizeHostname(
    hostnameFromUrl(pagesDevUrl) || `${config.projectName}.pages.dev`,
  );
  const marker = cloudflarePagesDnsMarker(
    projectId,
    config.projectName,
    pagesTarget,
  );
  ...
  try {
    ...
    return {
      ...base,
      status:
        details.errorCode === "cloudflare_dns_record_conflict"
          ? "conflict"
          : "failed",
  ...
  try {
    ...
    return {
      ...base,
      status:
        details.errorCode === "cloudflare_domain_already_bound"
          ? "conflict"
          : "failed",
      statusMessage: errorMessage(
        err,
        "Cloudflare Pages custom domain setup failed.",
      ),
  ...
  const customLink =
    domainStatus === "active" ? await checkDeploymentUrl(base.url) : null;
  ...
  return {
    ...
    statusMessage: ready
      ...
      : failed
        ...
        : customLink?.statusMessage ||
          "Custom domain is being verified by Cloudflare Pages.",
    ...
    domainStatus,
    pagesDomainStatus:
      typeof domain?.status === "string" ? domain.status : undefined,
...
async function ensureCloudflarePagesCnameRecord({
  ...
  if (conflicting) {
    if (
      ...
      if (!conflictingId)
        throw new DeployError("Cloudflare DNS record id is missing.", 502);
      const patched = await patchCloudflareDnsRecord(
        config,
        selection.zoneId,
        conflictingId,
        {
  ...
  try {
    ...
    if (
      !(err instanceof DeployError) ||
      !isCloudflareCommentError(err.details || err.message)
    )
      throw err;
    try {
      ...
      const racedRetryRecord =
        await maybeReuseCloudflarePagesCnameAfterDuplicate({
          err: retryErr,
          config,
          selection,
          targetHost,
...
function findExactCloudflarePagesCname(
...
function findCloudflarePagesHostnameRecord(
  records: CloudflareDnsRecord[],
  selection: CloudflarePagesDeploySelection,
) {
  return records.find(
    (record) => normalizeHostname(record?.name) === selection.hostname,
  );
}

function cloudflarePagesCnameReuseResult(
...
function cloudflarePagesDnsConflictError(
  selection: CloudflarePagesDeploySelection,
  conflicting: CloudflareDnsRecord,
) {
...
async function maybeReuseCloudflarePagesCnameAfterDuplicate({
  ...
  if (
    !(err instanceof DeployError) ||
    !isCloudflareAlreadyExists(err.details || err.message)
  )
    return null;
  const racedRecords = await listCloudflareDnsRecords(
    config,
    selection.zoneId,
    selection.hostname,
  );
  ...
  const conflicting = findCloudflarePagesHostnameRecord(
    racedRecords,
    selection,
  );
  if (conflicting)
...
async function patchCloudflareDnsRecord(
  config: DeployConfig,
  zoneId: string,
  dnsRecordId: string,
  body: JsonObject,
...
async function ensureCloudflarePagesDomain(
  config: DeployConfig,
  hostname: string,
) {
...
async function findCloudflarePagesDomain(
  config: DeployConfig,
  hostname: string,
) {
...
export async function readCloudflarePagesDomain(
  config: DeployConfig,
  hostname: string,
) {
  if (!config?.token)
...
function normalizeCloudflarePagesDomainStatus(status: unknown) {
  ...
  if (value === "error" || value === "blocked" || value === "deactivated")
    return "failed";
  return "pending";
}

export function aggregateCloudflarePagesStatus(
  pagesDev: JsonObject,
      ...
      statusMessage:
        pagesDev.status === "ready"
          ...
          : pagesDev.statusMessage ||
            "Cloudflare Pages is still preparing its pages.dev link.",
    };
  }
  if (customDomain.status === "pending") {
    return {
      status: "link-delayed",
      statusMessage:
        customDomain.statusMessage || "Custom domain is still being prepared.",
    };
  }
...
function cloudflarePagesProviderMetadata(
  projectName: string,
  cloudflarePagesInfo: JsonObject,
    ...
    ...(custom
      ? {
          cloudflarePagesCustomDomain: {
            ...
            marker: cloudflarePagesDnsMarker(
              projectId,
              projectName,
              hostnameFromUrl(cloudflarePagesInfo.pagesDev?.url),
            ),
...
async function getCloudflarePagesUploadToken(
  config: DeployConfig,
): Promise<string> {
...
async function uploadCloudflarePagesAssets(
  uploadToken: string,
  files: DeployFile[],
) {
  const uniqueFiles = new Map<
    string,
    { hash: string; data: Buffer; contentType: string }
  >();
  for (const file of files) {
  ...
  if (missing.length > 0) {
    ...
    for (const batch of chunkCloudflarePagesAssetUploads(missingFiles)) {
      ...
      const uploadResp = await fetch(`${CLOUDFLARE_API}/pages/assets/upload`, {
        method: "POST",
        headers: cloudflareAssetHeaders(uploadToken, {
          "Content-Type": "application/json",
        }),
  ...
  if (!upsertResp.ok || upserted?.success === false) {
    throw cloudflareError(
      upserted,
      upsertResp.status,
      "Cloudflare Pages asset hash update failed.",
    );
  }
}
...
export function chunkCloudflarePagesAssetUploads(
  ...
  for (const file of files) {
    ...
    const wouldExceedBytes =
      current.length > 0 && currentBytes + nextBytes > maxBytes;
...
function estimateCloudflarePagesAssetUploadPayloadBytes(file: {
  ...
  const contentTypeBytes = Buffer.byteLength(
...
async function cloudflarePagesMissingAssetHashes(
  uploadToken: string,
  hashes: string[],
): Promise<string[]> {
  ...
  if (!resp.ok || json?.success === false) {
    throw cloudflareError(
      json,
      resp.status,
      "Cloudflare Pages asset lookup failed.",
    );
  }
...
export function cloudflarePagesAssetHash(
  file: Pick<DeployFile, "file" | "data">,
) {
  const data = Buffer.from(file.data);
  const extension = path.posix.extname(file.file).slice(1);
  return blake3Hash(`${data.toString("base64")}${extension}`)
    .toString("hex")
    .slice(0, 32);
}
...
export function rewriteEntryHtmlReferences(html: string, baseDir: string) {
  ...
  return styleRewritten.replace(
    /<([A-Za-z][A-Za-z0-9:-]*)([^<>]*?)>/g,
    (tag, rawName, rawAttrs, offset) => {
...
function pushUnique(
  list: { warnings: JsonObject[]; seen: Set<string> },
  warning: JsonObject,
) {
...
export function analyzeDeployPlan(input: {
  ...
  const acc: { warnings: JsonObject[]; seen: Set<string> } = {
    warnings: [],
    seen: new Set(),
  };
  ...
  if (
    ...
    pushUnique(acc, {
      ...
      path: entryPath,
      message:
        "Entry HTML is missing `<!DOCTYPE html>`; browsers may render in quirks mode.",
    });
  }
  ...
  if (!hasViewport) {
    pushUnique(acc, {
      ...
      path: entryPath,
      message:
        'Entry HTML is missing `<meta name="viewport">`; mobile rendering will be off.',
    });
  }
...
export async function prepareDeployPreflight(
  projectsRoot: string,
  projectId: string,
  entryName: string,
  options: DeployOptions = {},
  ...
  return String(rawAttrs).replace(
    ...
    (full, rawName, equals, rawValue, doubleQuoted, singleQuoted, unquoted) => {
      ...
      if (name === "srcset") nextValue = rewriteSrcset(value, baseDir);
        ...
        nextValue = rewriteCssReferences(value, baseDir);
...
function cloudflarePagesProjectDomainUrl(
  config: DeployConfig,
  hostname: string,
) {
...
export function cloudflarePagesProjectNameForProject(
  projectId: string,
  projectName = "",
) {
  const idSuffix =
    safeDnsLabel(projectId).slice(0, 12) || randomUUID().slice(0, 8);
...
function cloudflareAssetHeaders(
  token: string,
  extra: Record<string, string> = {},
) {
  return {
...
function shouldFetchNextCloudflarePage(
  resultInfo: JsonObject | undefined,
  page: number,
  perPage: number,
  itemCount: number,
) {
  ...
  const effectivePerPage =
    Number.isFinite(responsePerPage) && responsePerPage > 0
      ? responsePerPage
      : perPage;
...
function cloudflarePagesDnsMarker(
  projectId: string,
  projectName: string,
  pagesTarget: string,
) {
...
function shortCloudflareHash(value: unknown) {
  return blake3Hash(String(value || ""))
    .toString("hex")
    .slice(0, 12);
}
