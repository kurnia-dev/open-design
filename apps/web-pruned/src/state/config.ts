...
function normalizeOrbit(input: Partial<OrbitConfig> | undefined): OrbitConfig {
  const time =
    typeof input?.time === "string" && isValidOrbitTime(input.time)
      ? input.time
      : DEFAULT_ORBIT.time;
...
export function loadConfig(): AppConfig {
  try {
    ...
    const merged: AppConfig = {
      ...
      accentColor:
        normalizeAccentColor(parsed.accentColor) ?? DEFAULT_CONFIG.accentColor,
    ...
    if (merged.apiProtocol) {
      merged.baseUrl = resolveFixedOriginBaseUrl(
        merged.apiProtocol,
        merged.baseUrl,
      );
    }
...
function hasRecoverableLocalMediaProviderFields(
  ...
  return Boolean(
    entry?.apiKey?.trim() || entry?.baseUrl?.trim() || entry?.model?.trim(),
  );
}
...
function defaultBaseUrlForProvider(providerId: string): string {
  return (
    MEDIA_PROVIDERS.find((provider) => provider.id === providerId)
      ?.defaultBaseUrl ?? ""
  );
}
...
export function buildMediaProvidersForDaemonSave(
  ...
  for (const [providerId, currentEntry] of Object.entries(
    currentProviders ?? {},
  )) {
    const daemonEntry = daemonProviders?.[providerId];
    const apiKey = currentEntry?.apiKey?.trim() ?? "";
    const hasStoredKeyMarker = Boolean(
      currentEntry?.apiKeyTail?.trim() || daemonEntry?.apiKeyTail?.trim(),
    );
    const preserveApiKey =
      !apiKey && Boolean(currentEntry?.apiKeyConfigured && hasStoredKeyMarker);
    const explicitBaseUrl =
      currentEntry?.baseUrl?.trim() || daemonEntry?.baseUrl?.trim() || "";
    const model =
      currentEntry?.model?.trim() || daemonEntry?.model?.trim() || "";
  ...
  for (const [agentId, env] of Object.entries(agentCliEnv)) {
    const safeEnv = Object.fromEntries(
      Object.entries(env ?? {}).filter(
        ([key]) => !AGENT_CLI_SECRET_ENV_KEYS.has(key),
      ),
    );
...
export function mergeDaemonConfig(
  ...
  if (daemonConfig.defaultProjectLocationId !== undefined) {
    next.defaultProjectLocationId =
      daemonConfig.defaultProjectLocationId ?? "default";
  }
  return next;
}
...
export function mergeDaemonMediaProviders(
  ...
  if (!hasAnyDaemonManagedMediaProvider(daemonProviders)) {
    return {
      ...
      mediaProviders: Object.fromEntries(
        Object.entries(localConfig.mediaProviders ?? {}).filter(
          ([, entry]) => !isMarkerOnlyMediaProviderEntry(entry),
        ),
      ),
    };
  }
  ...
  for (const [providerId, daemonEntry] of Object.entries(
    daemonProviders ?? {},
  )) {
