...
function eventStderrText(data: unknown): string[] {
  ...
  const payload =
    data && typeof data === "object" ? (data as Record<string, unknown>) : {};
  return [readString(payload.chunk), readString(payload.text)].filter(
...
function isHardQuotaText(text: string): boolean {
  return /\b(session limit|usage limit|limit reached|quota|billing (?:hard )?limit|insufficient[ _-]?(?:quota|credit|funds)|exceeded your current quota)\b/i.test(
    text,
  );
}

function isTimeoutText(text: string): boolean {
  return /\b(timed?\s*out|timeout|inactivity|stalled|hung|no new output|without emitting any new output)\b/i.test(
    text,
  );
}

function isEmptyOutputText(text: string): boolean {
  return /\b(empty response|empty output|without producing any output|no visible output|returned an empty response)\b/i.test(
    text,
  );
}
...
function isToolErrorText(text: string): boolean {
  return (
    /\b(tool|mcp|connector)\b/i.test(text) &&
    /\b(error|failed|failure)\b/i.test(text)
  );
}

function isPermissionRequestNotFoundText(text: string): boolean {
  return /\b(PermissionNotFoundError|Permission request not found|permissions\/per_[A-Za-z0-9_-]+\s+returned\s+HTTP\s+404)\b/i.test(
    text,
  );
}

function isAuthDetailText(text: string): boolean {
  return /\b(refresh token|access token could not be refreshed|stale local profile|different or stale local profile|missing environment variable: `?[A-Z0-9_]*API_KEY`?|api key.*missing|credentials? (?:are )?missing|not logged in)\b/i.test(
    text,
  );
}

function isPromptTooLargeText(text: string): boolean {
  return /\b(context window|prompt too large|maximum context|too many tokens|input.*too large|exceeds the safe size|composed prompt exceeds|prompt token count .* exceeds|maximum context length|reduce the length of (?:the )?(?:messages|input prompt))\b/i.test(
    text,
  );
}

function isUpstreamDetailText(text: string): boolean {
  return /\b(stream disconnected before completion|response\.completed|Transport error: network error|Upstream request failed|websocket closed|socket connection was closed unexpectedly|tls handshake eof|Connection reset by peer|TLS close_notify|Broken pipe|remote host|远程主机强迫关闭|No route to host|Connection refused|error sending request|Provider returned error|high demand|upstream_error|http2: response body closed)\b/i.test(
    text,
  );
}
...
function modelUnavailableDetail(text: string): TrackingRunFailureDetail | null {
  ...
  if (
    /\b(no endpoints found that support tool use|provider routing)\b/i.test(
      text,
    )
  ) {
  ...
  if (
    /\b(model .*not supported|requested model is not supported|supported api model names|not supported when using codex)\b/i.test(
      text,
    )
  ) {
  ...
  if (
    /\b(model (?:is )?(?:unavailable|not available|unsupported|not found)|selected model is not available|not have access|no access|model .*not found|no healthy deployments)\b/i.test(
      text,
    )
  ) {
...
function authDetail(text: string): TrackingRunFailureDetail {
  if (
    /\brefresh token (?:was )?(?:already used|expired|invalid)|access token could not be refreshed\b/i.test(
      text,
    )
  ) {
  ...
  if (
    /\b(stale local profile|different or stale local profile|stale or expired auth state|stale.*credential|stale.*profile)\b/i.test(
      text,
    )
  ) {
  ...
  if (
    /\bmissing environment variable: `?[A-Z0-9_]*API_KEY`?|api key.*missing|credentials? (?:are )?missing\b/i.test(
      text,
    )
  ) {
...
function upstreamDetail(text: string): TrackingRunFailureDetail {
  ...
  if (/\bhigh demand|temporary errors\b/i.test(text))
    return "provider_high_demand";
  if (
    /\b(stream disconnected before completion|response\.completed|websocket closed|socket connection was closed unexpectedly|connection reset|tls handshake eof|tls close_notify|broken pipe|peer closed connection|remote host|远程主机强迫关闭|http2: response body closed)\b/i.test(
      text,
    )
  ) {
  ...
  if (
    /\b(?:http|status|error|response)(?:[ _-]?code)?[\s:=#-]*5\d\d\b|\b5\d\d\s+(?:bad gateway|service unavailable|internal server error|gateway timeout)|\b(5xx|bad gateway|gateway timeout|internal server error|service unavailable|upstream[ _-](?:error|unavailable)|provider (?:error|unavailable)|overloaded)\b/i.test(
      text,
    )
  ) {
    return "upstream_5xx";
...
function processExitDetail(
  ...
  if (
    /\bnot installed|not on PATH\b/i.test(text) ||
...
export function classifyRunFailure(
  ...
  if (
    ...
    return classification(
      "timeout",
      /inactivity|stalled|hung|no new output|without emitting any new output/i.test(
        text,
      )
        ? "inactivity_timeout"
        : "timeout",
